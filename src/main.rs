    ...
    for i in 0..a_rows {
        for j in 0..b_cols {
            for k in 0..a_cols {
                ...
                println!("Elapsed time from within loop: {:?}", elapsed);
            }
        }
    }
...
fn main() {
    ...
    let b = norm_mat(4., 5., dims, dims);

    let start = Instant::now();
    let _ = matrix_multiply(&a, &b);
    let elapsed = start.elapsed();
    println!("Elapsed time from scratch: {:?}", elapsed);

    let start = Instant::now();
    let _ = a.dot(&b);
    let elapsed = start.elapsed();

    println!("Elapsed time from .dot : {:?}", elapsed);
}
